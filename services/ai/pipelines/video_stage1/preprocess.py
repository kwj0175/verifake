...
if shutil.which("ffmpeg") is None or shutil.which("ffprobe") is None:
    try:
        ...
        logging.getLogger(__name__).warning(
            "static_ffmpeg add_paths() failed during preprocess setup: %s",
            exc,
        )
...
def _normalize_video(input_path: Path, output_path: Path) -> None:
    if os.getenv("VERIFAKE_FAST_VIDEO_COPY", "1").strip().lower() not in {"0", "false", "no"}:
        shutil.copy2(input_path, output_path)
        return

    if shutil.which("ffmpeg") is None:
        logging.getLogger(__name__).warning(
            "ffmpeg binary is unavailable; skipping normalization and using source video as-is."
        )
        shutil.copy2(input_path, output_path)
        return
